const submitForm = async (event) => {
  ...
  if (location && startDate && finishDate) {
    ...
    if (!Client.checkCityName(location)) {
      ...
      // TODO: Display different UI when there is missing data
...
function dateDifferences(startDate, finishDate) {
  ...
  const differenceInDays = Math.abs(
    Number(Math.round(differenceInTime / (1000 * 3600 * 24)))
  );
...
function displayCityImg(cityInfo) {
  if (cityInfo.hits.length) {
    ...
    document.getElementById("city-img").src =
      "https://pixabay.com/get/ged565cd90cc2fc5ce4d0cd51fd058c8dd0309dced5bab3697e1b93ae04551a184c38827d89efe5c46c9495f9f4de93adec9dce2073340f40b08041c4210249fd_1280.jpg";
  }
}

function displayWeatherData(allWeatherData) {
  document.getElementById("city-name").innerHTML =
    allWeatherData.data[0].city_name;
  document.getElementById(
    "weather-icon"
  ).src = `./app/client/icons/${allWeatherData.data[0].weather.icon}.png`;
  document.getElementById(
    "weather-tem"
  ).innerHTML = `${allWeatherData.data[0].temp} °C`;
  document.getElementById("weather-des").innerHTML =
    allWeatherData.data[0].weather.description;
}

function displayTravelPeriod(travelPeriodData) {
  if (travelPeriodData === 1) {
    document.getElementById("travel-period").innerHTML =
      "The length of trip is " + travelPeriodData + " Day";
  } else {
    document.getElementById("travel-period").innerHTML =
      "The length of trip is " + travelPeriodData + " Days";
  }
}
